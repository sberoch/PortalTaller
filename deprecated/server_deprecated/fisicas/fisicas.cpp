...
#include "transformaciones/agregar_terreno.h"
...
void Fisicas::agregar(Terreno& unTerreno, Posicion& p) {
    transformaciones_.push(std::shared_ptr<Transformacion>(new AgregarTerreno(*this, unTerreno, p)));
}

void Fisicas::agregarPosta(Terreno& unTerreno, Posicion& p) {
	b2BodyDef groundBodyDef;
	groundBodyDef.position.Set(p.getX(), p.getY());
	groundBodyDef.userData = &unTerreno;
    b2Body* groundBody = mundo_->CreateBody(&groundBodyDef);
    unTerreno.forma()->inicializar(groundBody);
	identificables_[unTerreno.uuid()] = groundBody;
}
...
void Fisicas::teletransportarPosta(Entidad& unaEntidad, Posicion& p, Direccion& d) {
	...
	b2Cuerpo->SetTransform(b2Vec2(p.getX(), p.getY() + 1.4), b2Cuerpo->GetAngle());
